...
const colorCards = computed(() => {
  const colors = urlStore.data?.allColors;
  ...
  return [
    ...
    },
    {
      ...
      content:
        colors.light?.join("\n") ||
        "Для светлой темы цвета не найдены, возможно они в отделе 'Другие цвета'",
      colors: colors.light || [],
    },
    {
      type: "other",
      description: "Другие цвета",
      content: colors.other?.join("\n") || "",
      colors: colors.other || [],
    },
  ];
});
...
<template>
  <div class="baza">
    ...
    <div class="colors-section">
      <h2 class="page-description">Цвета</h2>
      <div class="color-theme-cards">
        <div
        ...
        >
          <div class="color-theme-header">
            <h3>{{ card.description }} ({{ card.colors.length }})</h3>
            <button
              class="copy"
              @click="copyItem(card.content, `color-card-${card.type}`)"
              :class="{ copied: copied[`color-card-${card.type}`] }"
            >
              <span>
                <svg
                  v-if="!copied[`color-card-${card.type}`]"
                  xml:space="preserve"
                  style="enable-background: new 0 0 512 512"
                  viewBox="0 0 6.35 6.35"
                  y="0"
          ...
          <div class="colors-scroll-container">
            ...
            >
              <button
                ...
                @click="copyItem(color, `color-${card.type}-${index}`)"
                :class="{ copied: copied[`color-${card.type}-${index}`] }"
              >
                <span>
                  <svg
                    v-if="!copied[`color-${card.type}-${index}`]"
                    xml:space="preserve"
                    style="enable-background: new 0 0 512 512"
                    viewBox="0 0 6.35 6.35"
                    y="0"
